...
class YingYongBaoSpider(RedisSpider):
    ...
    def __init__(self):
        ...
        # }
        self.headers = {
            'Referer': "https://sj.qq.com/myapp/detail.htm?apkName=com.crfchina.market",
            'Accept': 'application/json, text/javascript, */*; q=0.01',
            'Content-Type': 'application/json',
        }
    # 根据关键字构造初始请求
    def start_requests(self):
        ...
        # 遍历获取每一个关键字
        data = pd.read_csv("C:/File/code/app_class/app_info.csv")
        data_u = data.drop(index=data[data['app_name'].duplicated()].index, axis=1)
        #f = open('贷款app_list.txt','r',encoding='UTF-8')
        for name in data_u['app_name']:
        #for index in range(50001,keyword_total+1):
            ...
            #scrapy.FormRequest()
            yield scrapy.FormRequest(url=self.base_url, callback=self.get_page, formdata=formdata, meta={'keyword': self.keyword}, method="POST")
    # 解析页面函数
    def get_page(self,response):
        ...
        if response.status == 200:
            ...
            for appDetail in appDetails:  # 遍历列表获取每一条信息
                ...
                images = appDetail.get("images")  # 图片地址
                print("获取APP简介图片====================：", images)
                ...
                item["images"] = images if images else default_value  # APP图片链接
